...
export default class MultisigPropose extends Command {
  ...
  async run() {
    ...
    // Now serialize the propose action data manually too
    ...
    // trx (inline transaction, no length prefix — it's a struct, not bytes)
    ...
    try {
      // Use transact but with pre-serialized data (hex string)
      // The library won't try to re-serialize hex data fields
